...
/**
 * Drink Inventor — minimal, self-contained App.jsx
 * - No external deps
 * - Vite + React compatible
 * - Safe CSV export (RFC4180-style escaping)
 *
 * If you already have styles elsewhere, you can remove the <style> block below.
 */
...
// Simple seeded RNG so “Regenerate” changes but is stable for a given seed.
...
function makeSuggestions(options, count = 5, seed = Date.now()) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    // Choose 2–3 flavors from the selected set (fallback to global list)
...
function buildPitch({ name, spirit, flavors, sweetness, bubbly, glass, method }) {
  const sweetWord =
    sweetness < 30 ? "crisp and dry" : sweetness < 60 ? "balanced" : "lush";
...
export default function App() {
  ...
  const suggestions = useMemo(
    () => makeSuggestions(options, 5, seed),
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [spirit, sweetness, bubbles, flavorSet, seed]
  );
  ...
  const pitchesText = suggestions
    .map((s, i) => `${OPENERS[i % OPENERS.length]} ${s.pitch}`)
    .join("\n");
  ...
  function copyPitches() {
    navigator.clipboard.writeText(pitchesText);
  }

  function exportCSV() {
    downloadCSV("drink-inventor.csv", rowsForCSV);
  }
      ...
      <main className="grid">
        <section className="card">
          ...
          <label className="row">
            ...
            <textarea
              placeholder="Any constraints, allergens, venue vibes..."
              value={notes}
          ...
          <div className="actions">
            ...
            <button className="btn" onClick={copyPitches} type="button">
              Copy Pitches
            </button>
          ...
          {suggestions.map((s, i) => (
            <article className="suggestion" key={s.name + i}>
              <div className="s-title">
                ...
                <small>
                  {s.method} • {s.glass} • {s.bubbles !== "none" ? s.bubbles : "still"}
                </small>
              </div>
              <div className="s-body">
                ...
                <p className="ingredients">
                  <b>Build:</b> {s.ingredients}
                </p>
                <p className="pitch">
                  <b>{OPENERS[i % OPENERS.length]}</b> {s.pitch}
                </p>
              </div>
            </article>
          ))}
...
:root {
  ...
  --accent-2: #60a5fa; /* blue secondary */
  --border: #1f2430;
  --chip: #1a1f2a;
  --chip-active: #0f172a;
}
...
body {
  ...
  background: radial-gradient(1200px 800px at 10% -10%, #0f172a 0%, var(--bg) 55%);
  color: var(--text);
}
...
.chip.active {
  ...
  background: linear-gradient(180deg, var(--chip-active), rgba(94,234,212,0.08));
  box-shadow: 0 0 0 2px rgba(94,234,212,0.12) inset;
}

.actions { display: flex; gap: 10px; flex-wrap: wrap; margin-top: 12px; }
.btn {
  background: linear-gradient(180deg, rgba(96,165,250,0.15), rgba(94,234,212,0.12));
  border: 1px solid var(--border);
  color: var(--text);
  padding: 10px 14px;
  border-radius: 12px;
