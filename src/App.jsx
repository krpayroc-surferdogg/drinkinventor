...
function toCSV(rows){ if(!rows.length) return ""; const headers=Object.keys(rows[0]); const esc=v=>`\"${String(v).replace(/\"/g,'\"\"')}\"`; const lines=[headers.join(\",\")].concat(rows.map(r=>headers.map(h=>esc(r[h]??\"\")).join(\","))); return lines.join(\"\\n\"); }
...
export default function App(){
  ...
  const canGenerate=useMemo(()=>{ if(!allowCocktails&&!allowMocktails) return false; if(!totalRequested) return false; return true; },[allowCocktails,allowMocktails,totalRequested]);
...
function RetroBackdrop(){
  ...
  return (<div aria-hidden className=\"absolute inset-0 overflow-hidden\">
    {dots.map(d=> (<motion.div key={d.id} className=\"absolute\" style={{left:`${d.x}%`, top:`${d.y}%`}} animate={{y:[0,-6,0]}} transition={{repeat:Infinity, duration:6+Math.random()*4, delay:Math.random()*2}}><svg width={d.size} height={d.size} viewBox=\"0 0 10 10\" className=\"opacity-40\"><circle cx=\"5\" cy=\"5\" r=\"5\" fill={d.color}/></svg></motion.div>))}
    {slices.map(s=> (<motion.div key={s.id} className=\"absolute\" style={{left:`${s.x}%`, top:`${s.y}%`, rotate:s.rot}} animate={{rotate:[s.rot,s.rot+10,s.rot]}} transition={{repeat:Infinity, duration:14+Math.random()*6}}><CitrusIcon size={64} color={s.color} /></motion.div>))}
    {leaves.map(l=> (<motion.div key={l.id} className=\"absolute\" style={{left:`${l.x}%`, top:`${l.y}%`, rotate:l.rot}} animate={{y:[0,4,0]}} transition={{repeat:Infinity, duration:10+Math.random()*6}}><LeafIcon size={34} color={l.color} /></motion.div>))}
    {glasses.map(g=> (<motion.div key={g.id} className=\"absolute text-slate-800/30\" style={{left:`${g.x}%`, top:`${g.y}%`}} animate={{y:[0,-8,0]}} transition={{repeat:Infinity, duration:12+Math.random()*6}}>
...
function runTests(){
  ...
  try{ const rows=[{A:'x',B:'y'},{A:'1,2',B:'He said \"Hi\"'}]; const csv=toCSV(rows); const expected=['A,B','\"x\",\"y\"','\"1,2\",\"He said \"\"Hi\"\"\"'].join('\\n'); cases.push({name:'toCSV()',passed:csv===expected}); }catch(e){ cases.push({name:'toCSV() threw',passed:false,details:String(e)}); }
  try{ const r=composeRecipe({theme:'Summer',holiday:'None',flavors:['Citrusy'],spirits:['Any'],allowCocktails:true,allowMocktails:true,tier:'Regular'}); const keys=['name','tier','isMocktail','base','technique','glass','garnish','ingredients','spec','method','description','notes']; const hasAll=keys.every(k=>Object.prototype.hasOwnProperty.call(r,k)); const descOk=typeof r.description==='string' && r.description.length>25; cases.push({name:'composeRecipe() + description',passed:hasAll && Array.isArray(r.spec) && r.spec.length>=3 && descOk}); }catch(e){ cases.push({name:'composeRecipe() threw',passed:false,details:String(e)}); }
  try{ const tips=elevationTips('Margarita'); const hasMezcal=tips.some(t=>/mezcal/i.test(t.tip)); cases.push({name:'elevationTips() Margarita contains mezcal split tip',passed:hasMezcal}); }catch(e){ cases.push({name:'elevationTips() threw',passed:false,details:String(e)}); }
  try{ const ok=[Card,Label,Select,Chip,Toggle,Num,GradButton,GhostButton,TierBadge].every(fn=>typeof fn==='function'); cases.push({name:'UI primitives exist', passed:ok}); }catch(e){ cases.push({name:'UI primitives exist check threw', passed:false, details:String(e)}); }
  return cases;
}
